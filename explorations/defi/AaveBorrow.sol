...
contract yDelegatedVault is ERC20, ERC20Detailed {
    ...
    constructor (address _token) public ERC20Detailed(
        string(abi.encodePacked("opty ", ERC20Detailed(_token).name())),
        string(abi.encodePacked("op", ERC20Detailed(_token).symbol())),
    ...
    function debt(address _borrowToken) public view returns (uint) {
        (,uint currentBorrowBalance,,,,,,,,) = Aave(getAave()).getUserReserveData(_borrowToken, address(this));
    ...
    function locked(address _borrowToken, uint _borrowAmount) public view returns (uint) {
        return _borrowAmount.mul(1e18).div(debt(_borrowToken));
    }
    
    function debtShare(address _borrowToken, address _lp) public view returns (uint) {
        return debt(_borrowToken).mul(balanceOf(_lp)).mul(totalSupply());
    }
    ...
    function repayAll(address _borrowToken) public {
        uint _amount = IERC20(_borrowToken).balanceOf(address(this));
        repay(_borrowToken, _amount);
    }
    ...
    // maxETH 13538152959967579
    // eth    1805051698230649
    
    function over(address _borrowToken, uint _amount) public view returns (uint) {
        ...
        if (_maxSafeETH < _totalBorrowsETH) {
            ...
            _over = _over.mul(uint(10)**ERC20Detailed(_borrowToken).decimals()).div(getReservePrice(_borrowToken));
            return _over;
        } else {
            return 0;
        }
    ...
    function _rebalance(address _borrowToken, uint _aTokenAmount, uint _borrowAmount) internal {
        uint _over = over(_borrowToken,_aTokenAmount);
        if (_over > 0) {
            if (_over > _borrowAmount) {
                _over = _borrowAmount;
            }
            if (_over > 0) {
                IERC20(_borrowToken).safeApprove(getAaveCore(), 0);
                IERC20(_borrowToken).safeApprove(getAaveCore(), _over);
                repayAll(_borrowToken);
            }
        }
    }
    
    function rebalance(address _borrowToken, uint _aTokenAmount) external {
        _rebalance(_borrowToken, _aTokenAmount,0);
    }
    ...
    function availableToBorrowReserve(address _borrowToken) public view returns (uint) {
        uint _available = availableToBorrowETH();
        if (_available > 0) {
            return _available.mul(uint(10)**ERC20Detailed(_borrowToken).decimals()).div(getReservePrice(_borrowToken));
        } else {
            return 0;
        }
    }
    
    function getReservePrice(address _borrowToken) public view returns (uint) {
        return getReservePriceETH(_borrowToken);
    }
    ...
    function earn(address _borrowToken) external {
        uint _borrow = availableToBorrowReserve(_borrowToken);
        if (_borrow > 0) {
            Aave(getAave()).borrow(_borrowToken, _borrow, 1, 0);
        }
    }
    
    function balanceOfBorrowToken(address _borrowToken) public view returns(uint) {
        return IERC20(_borrowToken).balanceOf(address(this));
    }
    ...
    function withdrawAll(address _borrowToken, uint _borrowAmount) public {
        withdraw(_borrowToken,balanceOf(msg.sender),_borrowAmount);
    }
    
    // Calculates in impermanent lock due to debt
    function maxWithdrawal(uint _aTokenAmount,address _borrowToken, uint _borrowAmount) public view returns (uint) {
        uint _safeWithdraw = _aTokenAmount.mul(locked(_borrowToken,_borrowAmount)).div(1e18);
    ...
    function safeWithdraw(address _borrowToken) external {
        uint _borrowAmount = IERC20(_borrowToken).balanceOf(address(this));
        uint _aTokenAmount = IERC20(token).balanceOf(address(this));
        withdraw(_borrowToken,maxWithdrawal(_aTokenAmount,_borrowToken,_borrowAmount),_borrowAmount);
    }
    
    // No rebalance implementation for lower fees and faster swaps
    function withdraw(address _borrowToken, uint _aTokenAmount,uint _borrowAmount) public {
        _rebalance(_borrowToken, _aTokenAmount,_borrowAmount);
        token.safeTransfer(msg.sender, _aTokenAmount);
    }
}
